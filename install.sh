...
include_templates() {
    templateType="$1"
    echo "Checking for templates files of type: $templateType..."
    ...
    do
        ...
        if read -r templateHeader < "$templateFile"
            then
                if [ "$templateHeader" = "$PSH_TEMPLATE" ]
                    then
...
include_templates "$TEMPLATE_BETWEEN_ANTIGEN_AND_OH__"
...
include_templates "$BETWEEN_OH_MY_ZSH_AND_PLUGINS"
