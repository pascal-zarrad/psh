...
if [ -f "${ANTIDOTE_PATH}" ]; then
    ...
    if git clone --depth=1 https://github.com/mattmc3/antidote.git "${ANTIDOTE_FOLDER_PATH}"; then
        ...
        print_error "Failed to install antidote!"
        exit 1
    fi
fi
...
# Now reset ~/.zshrc, as we build our own only using antidote
# to load things
{
...
{
    echo "# Load antidote"
    echo "source ${ANTIDOTE_PATH}"
} >>"${ZSHRC_PATH}"

# Include templates between oh-my-zsh and the plugin execution
include_templates "${TEMPLATE_BETWEEN_ANTIDOTE_SOURCE_AND_PLUGINS}" "${start_arg_disable_template_engine_parameter}" "${ZSHRC_PATH}"
...
if [ "${start_arg_disable_plugin_system_parameter}" -ne 1 ]; then
    ...
    # plugins are loaded.
    # Also ensures file is always written from scratch
    {
        echo "# Load plugins and themes (generated by psh plugins during installation)"
    } >"${ANTIDOTE_PLUGINS_LIST_PATH}"

    # Add oh my zsh including libraries by default
    # Unlike plugin managers used in previous PSH version,
    # antidote is more sensitive regarding the order of loading plugins.
    apply_ohmyzsh_initialization
...

{
    ...
    echo "# Load plugins using antidote"
    echo "antidote load ${ANTIDOTE_PLUGINS_LIST_PATH} ${ANTIDOTE_STATIC_PLUGINS_PATH}"
} >>"${ZSHRC_PATH}"
