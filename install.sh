...
if [ -f "${CUSTOM_USER_HOME_DIR}/.antigen/antigen.zsh" ]
    then
        print_success "Antigen is already installed."
    else
        if ! [ -d "${CUSTOM_USER_HOME_DIR}/.antigen" ]; then
            mkdir "${CUSTOM_USER_HOME_DIR}/.antigen"
        fi
        if curl -L git.io/antigen > "${CUSTOM_USER_HOME_DIR}/.antigen/antigen.zsh"
            then
                print_success "Successfully installed antigen to ${CUSTOM_USER_HOME_DIR}/.antigen/antigen.zsh"
            else
...
print_message ""
print_message "Backing up ${CUSTOM_USER_HOME_DIR}/.zshrc to ${CUSTOM_USER_HOME_DIR}/.zshrc_unmodified..."
if [ -f "${CUSTOM_USER_HOME_DIR}/.zshrc" ]
    then
        if cp "${CUSTOM_USER_HOME_DIR}/.zshrc" "${CUSTOM_USER_HOME_DIR}/.zshrc_unmodified"
            then
                print_success "Backed up ${CUSTOM_USER_HOME_DIR}/.zshrc"
            else
                print_error "Failed to backup ${CUSTOM_USER_HOME_DIR}/.zshrc"
        fi
    else
...
print_message "Preparing ${CUSTOM_USER_HOME_DIR}/.zshrc..."
...
# to load things
{
    ...
    echo "# ======================================"
} > "${CUSTOM_USER_HOME_DIR}/.zshrc"

# Include templates the the start of the file after the header
include_templates "$TEMPLATE_START" "$start_arg_disable_template_engine_parameter" "$CUSTOM_USER_HOME_DIR"

# Enable antigen
{
    echo "# Enable antigen"
    echo "source ${CUSTOM_USER_HOME_DIR}/.antigen/antigen.zsh"
} >> "${CUSTOM_USER_HOME_DIR}/.zshrc"

# Include templates after antigen has been loaded but before oh-my-zsh is being loaded
include_templates "$TEMPLATE_BETWEEN_ANTIGEN_AND_OH_MY_ZSH" "$start_arg_disable_template_engine_parameter" "$CUSTOM_USER_HOME_DIR"
...
include_templates "$TEMPLATE_BETWEEN_OH_MY_ZSH_AND_PLUGINS" "$start_arg_disable_template_engine_parameter" "$CUSTOM_USER_HOME_DIR"

print_success "Prepared ${CUSTOM_USER_HOME_DIR}/.zshrc"
...
include_templates "$TEMPLATE_AFTER_PLUGINS_BEFORE_ANTIGEN_APPLY" "$start_arg_disable_template_engine_parameter" "$CUSTOM_USER_HOME_DIR"
...
include_templates "$TEMPLATE_END" "$start_arg_disable_template_engine_parameter" "$CUSTOM_USER_HOME_DIR"
