...
function check_brew_installed() {
    if ! brew --version &>/dev/null; then
        print_error "You need Homebrew installed to install psh on macOS"
        print_error "Checkout https://brew.sh to find out how to install Homebrew!"
        exit 1
    fi
}
...
function packages_installed() {
    ...
    for dependency in "${dependencies[@]}"; do
        brew_list_check_result=$(brew list 2>/dev/null | grep -c "$dependency")
        if [ "$brew_list_check_result" -eq 0 ]; then
...
function install_apt_packages() {
    ...
    print_message "The installer has to install the following packages through brew: "
    ...
    brew install "${package_install_arguments[@]}"
    install_result="$?"
